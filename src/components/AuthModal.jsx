...
function SignUpView({ onSwitchToLogin, onVerificationRequired }) {
  ...
  return (
    <form className="auth-modal__form auth-modal__form--signup" onSubmit={handleSubmit} noValidate>
...
export default function AuthModal({ isOpen, defaultTab = 'signup', onClose, onSuccess }) {
  ...
  const heroContent = verificationEmail
    ? {
    ...
    : activeTab === 'signup'
      ? {
          title: 'Create your Orixus account.',
          subtitle: 'Start building consistency today.',
        }
      : {
          title: 'Welcome back.',
          subtitle: 'Continue building discipline.',
        };
    ...
    >
      <div className="auth-modal">
        ...
        <header className="auth-modal__hero">
          <h1 className="auth-modal__hero-title">{heroContent.title}</h1>
          <p className="auth-modal__hero-subtitle">{heroContent.subtitle}</p>
        </header>
